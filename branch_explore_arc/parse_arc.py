...
BOOKISH_ZHCN_REFERENCE_EPUB_FILE = ROOT / "bookish_complete.zh.epub"
BOOKISH_ZHCN_EPUB_FILE = ROOT / "死神与少女_中文完整剧本.epub"
...
BOOKISH_ZHCN_EPUB_TITLE = "死神与少女 中文完整剧本"
BOOKISH_ZHCN_EPUB_AUTHOR = "藤文/takuyo"
...
BOOKISH_RECOVERY_ROUTE_CHAPTERS = {
    ...
    "recovery_4_37_6_03": ["序章", "一章", "二章", "三章", "四章", "五章"],
    "ao39_2": ["蒼の章"],
}
...
def write_bookish_zhcn_readme(
    ...
    lines = [
        ...
        "- Translation source: `bookish/bookish_complete.ja.zh.epub` is the only default JP/ZH source.",
        ...
        f"- Legacy fallback loading: {'enabled' if translator.stats['legacy_fallbacks_enabled'] else 'disabled'}",
        f"- Legacy source files loaded: {translator.stats['source_files']}",
        f"- Legacy line pairs loaded: {translator.stats['paired_lines']}",
        ...
        f"- `{BOOKISH_ZHCN_REFERENCE_EPUB_FILE.relative_to(ROOT).as_posix()}` is the frozen reference EPUB packaged from the reviewed `bookish_zhcn` Markdown.",
        f"- New zh-CN EPUB generation writes `{BOOKISH_ZHCN_EPUB_FILE.relative_to(ROOT).as_posix()}` so the reference EPUB is not overwritten.",
        "",
        "## Chapter Files",
        "",
    ]
...
def remove_dialogue_terminal_periods(text: str) -> str:
    return text.replace("。」", "」")
...
def sanitize_translation_text(text: str) -> str:
    ...
    cleaned = remove_dialogue_terminal_periods(cleaned)
    return cleaned
...
def format_untranslated_dialogue(speaker: str, content: str) -> str:
    return f"{normalize_bookish_name(speaker)}: 「{strip_interactive_markers(content).strip()}」"
...
def authoritative_epub_aliases_for_source_file(source_file: str | None) -> list[str]:
    if not source_file:
        return []
    exact = {
        "prologue/name.txt": "00_hajimari.xhtml",
        "prologue/hajimari.txt": "00_hajimari.xhtml",
    }
    prefixes = [
        ("prologue/", "01_prologue.xhtml"),
        ("chapter1/", "02_chapter1.xhtml"),
        ("chapter2/", "03_chapter2.xhtml"),
        ("chapter3/", "04_chapter3.xhtml"),
        ("natsuko/", "05_natsuko.xhtml"),
        ("chapter4/", "06_chapter4.xhtml"),
        ("hinase/", "07_hinase.xhtml"),
        ("chapter5/", "08_chapter5.xhtml"),
        ("kirishima/", "09_kirishima.xhtml"),
        ("chiyo/", "10_chiyo.xhtml"),
        ("chapter6/", "11_chapter6.xhtml"),
        ("chapter7_kuro/", "12_kuro.xhtml"),
        ("chapter7_ao/", "13_ao.xhtml"),
        ("ending/", "14_atogaki.xhtml"),
    ]
    filename = exact.get(source_file)
    if not filename:
        for prefix, candidate in prefixes:
            if source_file.startswith(prefix):
                filename = candidate
                break
    return [f"authoritative_epub/{filename}"] if filename else []
...
class BookishTranslator:
    def __init__(
        ...
        authoritative_epub: Path = BOOKISH_JA_ZH_EPUB_FILE,
        allow_legacy_fallbacks: bool = False,
    ) -> None:
        self.processed_dir = processed_dir
        self.translated_dir = translated_dir
        self.authoritative_epub = authoritative_epub
        self.allow_legacy_fallbacks = allow_legacy_fallbacks
        ...
        self.stats = {
            ...
            "legacy_fallbacks_enabled": allow_legacy_fallbacks,
        }
    ...
    def _load_authoritative_epub(self) -> None:
        if not self.authoritative_epub.exists():
            return
        ...
        with zipfile.ZipFile(self.authoritative_epub) as archive:
    ...
    def _pick_candidate(
        ...
        if source_file:
            for candidate_source in [source_file, *authoritative_epub_aliases_for_source_file(source_file)]:
                source_candidates.extend(by_file_map.get(candidate_source, {}).get(key, []))
        candidates.extend(source_candidates)

        allow_global_candidates = True
        if source_file and not source_candidates and len(normalize_translation_key(source)) < 12:
            allow_global_candidates = False
        if allow_global_candidates:
    ...
    def translate_line(self, line: str, source_file: str | None = None) -> str:
        ...
        if source_dialogue:
            ...
            if not content:
                full = self.lookup(line, source_file) if len(normalize_translation_key(source_content)) >= 12 else None
            ...
            return format_untranslated_dialogue(speaker, source_content)
...
BOOKISH_READER_NAVIGATION_LINKS = {
    ...
    "reading_order/17_chapter6.md": [
        ("进入黑之章：", "黑之章", "reading_order/18_kuro.md"),
        ("进入苍之章：", "苍之章", "reading_order/19_ao.md"),
    ],
    "reading_order/18_kuro.md": [("跳至：", "苍之章", "reading_order/19_ao.md")],
...
def markdown_to_epub_xhtml(
    ...
    def force_page_after_navigation_if_needed() -> None:
        nonlocal last_visible_block_was_navigation
    ...
    for raw_line in markdown.splitlines():
        ...
        if anchor_match:
            close_unordered_list()
            force_page_after_navigation_if_needed()
...
def write_bookish_complete_markdown(
    ...
    lines: list[str] = [
        f"# {title}",
        "",
        "_EPUB-ready complete Markdown generated from reading-order files._",
        "",
    ]
...
def write_epub_styles(epub_dir: Path) -> None:
    ...
    legacy_style = styles_dir / "bookish.css"
    if legacy_style.exists():
        legacy_style.unlink()
    (styles_dir / "reader.css").write_text(
        "\n".join(
            [
                "body {",
                "  font-family: serif;",
...
def write_bookish_outputs(data: dict[str, Any]) -> None:
    ...
    zhcn_epub_outputs = write_bookish_epub(
        ...
        author=BOOKISH_ZHCN_EPUB_AUTHOR,
        identifier_suffix="shinigami-to-shoujo-zhcn",
    ...
    write_json(
        BOOKISH_ZHCN_DIR / "manifest.json",
        {
            ...
            "translation_source": {
                "mode": "authoritative_epub_only"
                if not translator.stats["legacy_fallbacks_enabled"]
                else "authoritative_epub_with_legacy_fallbacks",
                ...
                "legacy_fallbacks_enabled": translator.stats["legacy_fallbacks_enabled"],
                "legacy_sources": {
                ...
                "legacy_loaded_source_files": translator.stats["source_files"],
                "legacy_loaded_line_pairs": translator.stats["paired_lines"],
                "legacy_missing_translated_files": translator.stats["missing_translated_files"],
            },
            "file_structure": "original_chapter_count",
            "chapter_files": chapter_file_specs_for_manifest(),
            ...
            "reference_epub": {
                "path": BOOKISH_ZHCN_REFERENCE_EPUB_FILE.relative_to(ROOT).as_posix(),
                "role": "Frozen reviewed reference EPUB. New zh-CN generation writes the epub.epub_file path instead.",
            },
